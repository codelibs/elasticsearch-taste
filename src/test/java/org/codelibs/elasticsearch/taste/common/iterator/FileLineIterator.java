...
package org.codelibs.elasticsearch.taste.common.iterator;
...
import org.codelibs.elasticsearch.taste.common.SkippingIterator;
 ...
 */
public final class FileLineIterator extends AbstractIterator<String> implements
        SkippingIterator<String>, Closeable {

    private final BufferedReader reader;

    private static final Logger log = LoggerFactory
            .getLogger(FileLineIterator.class);
     ...
     */
    public FileLineIterator(File file, boolean skipFirstLine)
            throws IOException {
     ...
     * @throws IOException                   if the file cannot be read
     */
    public FileLineIterator(File file, Charset encoding, boolean skipFirstLine)
            throws IOException {
    ...
    public FileLineIterator(InputStream is, boolean skipFirstLine)
            throws IOException {
        this(is, Charsets.UTF_8, skipFirstLine);
    }

    public FileLineIterator(InputStream is, Charset encoding,
    ...
    public FileLineIterator(InputStream is, Charset encoding,
            boolean skipFirstLine, String filename) throws IOException {
        InputStream compressedInputStream;

        if ("gz".equalsIgnoreCase(Files.getFileExtension(filename.toLowerCase()))) {
            compressedInputStream = new GZIPInputStream(is);
        } else if ("zip".equalsIgnoreCase(Files.getFileExtension(filename
                .toLowerCase()))) {
        ...
        reader = new BufferedReader(new InputStreamReader(
                compressedInputStream, encoding));
        if (skipFirstLine) {
            reader.readLine();
        }
    }
