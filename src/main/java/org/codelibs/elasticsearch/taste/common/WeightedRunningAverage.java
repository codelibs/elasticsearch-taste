...
public class WeightedRunningAverage implements RunningAverage, Serializable {
    ...
    public synchronized void addDatum(final double datum, final double weight) {
        final double oldTotalWeight = totalWeight;
        totalWeight += weight;
    ...
    public synchronized void removeDatum(final double datum, final double weight) {
        final double oldTotalWeight = totalWeight;
        totalWeight -= weight;
        if (totalWeight <= 0.0) {
            average = Double.NaN;
    ...
    @Override
    public synchronized void changeDatum(final double delta) {
        changeDatum(delta, 1.0);
    }

    public synchronized void changeDatum(final double delta, final double weight) {
        Preconditions.checkArgument(weight <= totalWeight,
                "weight must be <= totalWeight");
